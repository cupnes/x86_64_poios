...
加えて、この章で実装するKBC関連の機能の構成は@<img>{3_kbc}の通りです。

//image[3_kbc][この章で作る機能の構成]{
//}
...
@<list>{inline_asm_ioformat}の"使用するレジスタ等の指定"には、in命令の仕様の都合上、入力オペランドには"d"のレジスタを、出力オペランドには"a"のレジスタを指定しています。
...
//footnote[about_64bit_reg_name][64ビットモードでは汎用レジスタ自体も16個へ増えており、増えた分は"R8"〜"R15"という名前になっています。そのため、RAX等の'R'は単に"Register"を指すものと思われます。]
...
まずは、「指定したIOアドレスのレジスタから値を取得する」処理をC言語から汎用的に呼べるように関数化します。本書では、インラインアセンブラを使用して"io_read"という関数を用意することにします。x86 CPUに依存した関数群は"x86.c"というソースファイルにまとめることにし、対応して"x86.h"というヘッダファイルも用意します(@<list>{030_x86_h}、@<list>{030_x86_c})。
