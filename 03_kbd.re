= キーボード入力
文字を出力することはできたので、次は順当にキーボードから文字の入力を取得してみます。

もはやレガシーなデバイスとなりますが、本書ではキーボードコントローラ(KBC)からキー入力を取得してみます。レガシーデバイスではありますが、QEMUと実機@<fn>{03_jikki}で今でも使用できます。
//footnote[03_jikki][Lenovo ThinkPad E450で確認]
