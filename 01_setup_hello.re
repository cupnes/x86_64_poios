...
UEFIについてはGoogle検索でもいろいろと情報が出てきますが、「UEFIファームウェアを直接叩くプログラムを作る」本を出しており、PDF版は無料で公開していますので興味があれば見てもらえると嬉しいです(本書の「参考情報」にURLを記載しています)。
//}

== ブートローダを用意
本書で作成するプログラムをメモリへロードし、そこへジャンプする「ブートローダ」は既に用意したものがあります。

以下のURLからブートローダ"poiboot"のアーカイブ"poiboot_20180422.zip"をダウンロードして展開しておいてください。
...
ここで実行されるプログラムが一般的にブートローダと呼ばれるものです。ブートローダはファームウェアとやり取りして、その次に実行するプログラム(一般的には「カーネル」)をメモリへロードし、ファームウェアとのやり取りを終え@<fn>{why_exit_firmware}、メモリへロードしたプログラムの実行を開始します。
...
本書は64ビットのx86CPUの制御方法を紹介する本ですので、UEFIは終了した状態から始めるため、ブートローダは上記の通り、既に用意したものを使って説明します。

なお、上記のpoibootは前著「フルスクラッチで作る!UEFIベアメタルプログラミング パート1/パート2」の内容に少し追加したものです。ブートローダに興味があれば見てみてください(無料公開しているPDF版のURLを本書「参考情報」に記載しています)。また、前著から手を加えた部分は本書「付録」に記載しています。

== その他
本書のプログラムを実機で試したい場合はUSBフラッシュメモリ等、PCの起動ディスクとして使えるストレージをご用意ください。

「UEFI + x86(64ビット)」なPCであれば動作すると思いますが、筆者の手持ちの都合上「Lenovo製ThinkPad E450」でしか確認していないため、他のPCでは動かないことがあるかも知れません。

主にブートローダにて、PCに依存しそうな設定箇所は、本書「付録」に記載していますので、実機で動かない時はこちらを見てみてください。
