...
=== poibootをダウンロード
以下のURLからpoibootのアーカイブ"poiboot_20180422.zip"をダウンロードし、展開しておいてください。

 * @<href>{http://yuma.ohgami.jp}

poiboot_20180422.efiを何処に配置して使用するかは次章で最初のプログラムを作っていく中で紹介します。

=== 設定ファイル(poiboot.conf)について
poiboot準備の最後として、設定ファイル(poiboot.conf)を用意します。

設定内容は、poibootがロードするカーネルバイナリ(kernel.bin)とファイルシステムイメージ(fs.img)それぞれのロード先アドレスだけです。

なお、poibootのアーカイブにはデフォルト値記載済のpoiboot.confが入っていますので、「デフォルトのpoiboot.confで試し、ダメだったらメモリマップ確認(後述)を行う」でも良いかと思います@<fn>{01_sekkaku}。QEMUは仕様が変わらない限りメモリマップは変わらないと思いますし、実機の場合も後述のメモリマップ確認方法で確認した結果が公開されているのをいくつか見る限りは問題無いかと思います。
