...
この章ではCPU側の設定とPICの使い方を説明し、前章のキー入力を割り込みを使用して行うようにしてみます。

この章全体を通してサンプルディレクトリ"040_intr"の内容を説明します。
...
64ビットモードでの動作時、一部のCPU命令にはスタックポインタ(RSP)が16の倍数になっていること(16バイトアライメント)を前提にコンパイラが機械語コードを生成するものがあります。そのため、64ビットモードにおいて、コンパイラは、関数実行開始時のRSPが16バイトアライメントされるように機械語コードを生成します。C言語の世界の中で関数を呼び出したり呼びだされたりしている間はコンパイラがよしなに調節してくれるので16バイトアライメントを気にする必要はありません。ただし、今回の割り込みハンドラの様に自力でアセンブラを書いて関数呼び出しを行う場合は注意が必要です。

なお、割り込み発生時、CPUはRSPを16バイトアライメント調節してから割り込みハンドラを呼び出してくれます。

注意しなければならないのはcall命令の挙動です。call命令は、1)戻り先アドレス(8バイト)をスタックに積む(RSPは-8される)、2)呼び出し先の関数へジャンプ、という挙動です。そのため、割り込みハンドラ先頭では16バイトアライメントであったとしても、call命令の挙動により呼び出し先の関数先頭ではRSPは8引いた値となり、16バイトアライメントは崩れてしまいます。
