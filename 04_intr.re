...
この章ではCPUの割り込み設定方法とPICの使い方を説明し、前章のキー入力を割り込みを使用して行うようにしてみます。章全体を通してサンプルディレクトリ"040_intr"の内容を説明します。
...
DS(データセグメント)・SS(スタックセグメント)のレジスタ設定は、単にmov命令を使用してレジスタへ値を格納するだけです。ただしCS(コードセグメント)設定は、現在実行中のCSを切り替えることになるため、例外へ陥ることが無いよう、「予めスタックに設定値をpushしret(関数リターン命令)で設定」という方法で設定します(@<list>{040_set_cs_ds_ss})。
...
//footnote[kbc_interrupt_not_better][一般的には割り込みハンドラで本格的な処理を行うべきでは無いです。(割り込みはシステム全体を止めるため)しかし、ここでは気にせず、割り込み契機で行いたい事は全てハンドラに実装してみることにします。(今はまだスケジューラも無く、ハンドラの処理が長引くことで被害を受ける人も居ないので。)]
...
ここまででIDTの設定も完了で、セグメンテーション含めてCPUの割り込み有効化のための設定は完了です。
...
　@<table>{pic_ioaddr}のICW4のbit4の"SFNM"は「特殊完全ネストモード」と呼ばれるモードの有効/無効の設定です。スレーブから多重に割り込みを受け付けるモードを使用するか否かを設定します。
