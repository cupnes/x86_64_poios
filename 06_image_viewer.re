= 画像ビューアアプリを作ってみる
「アプリ」といってもバイナリレベルでカーネルと分けたり、動作時の権限を変えたりは本書ではまだしません。これまでプラットフォームとして実装してきた「フレームバッファ」・「キーボードコントローラ」・「割り込み」・「ファイルシステム」等の道具を使用して何らかの用途を実現するサンプルとして、本書では画像のビューアを作ってみます。
...
この中で、「サイズ(幅・高さ)」のみ具体的な値がまだわかっていません。
...
===[column] KBCドライバが画像ビューアを呼び出すのは微妙
趣味で書くOSは何をどうやっても自由なので、設計についても自分なりに考えてみるのは面白いです。

今回はやりたいことを実現する手っ取り早い方法としてkbc.c(KBCドライバ)内のKBC割り込みのハンドラから画像ビューアの割り込み処理を呼び出すようにしましたが、設計としては微妙な気がします。

KBCドライバはKBCを管理・制御する事に従事すべきで、「画像ビューア」等という上位のアプリケーション(カーネルと一緒になってしまってはいますが)と直接お話しするべきでは無いかと思います。ドライバが直接やり取りする対象はカーネルなので、修正するとすれば、例えば、「カーネル側に割り込み時のコールバック関数登録機能を用意し、アプリケーションはカーネルへ『この関数を割り込み時に呼び出してくれ』とお願いする」等でしょうか。

もっと考えると、ハードウェアの機能である「割り込み」をアプリケーションに扱わせるのも微妙かもしれません。アプリケーションは「割り込みを使いたい」のではなく「データが欲しい」場合があります。今回の場合、「キー入力があったことと入力値」を知ることができれば割り込みでも何でも構わないのです。そのため、C言語でアプリケーションを作るときのgetc関数のように「アプリケーションへは当該データを得るためのインタフェースを提供し、割り込み等の処理はカーネル内へ隠蔽する」というのも良いかと思います。
