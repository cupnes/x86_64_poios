= 画像ビューアアプリを作ってみる
「アプリ」といってもバイナリレベルでカーネルと分けたり、動作時の権限を変えたりは本書ではまだしません。これまでプラットフォームとして実装してきた「フレームバッファ」・「キーボードコントローラ」・「割り込み」・「ファイルシステム」等の道具を使用して何らかの用途を実現するサンプルとして、本書では画像のビューアを作ってみます。作成する画像ビューアの構成は@<img>{6_iv}の通りです。
...
実行すると、画像が表示され、'j'キー/'k'キーで画像送り/戻りが確認できます(@<img>{063_iv_image_viewer_1}と@<img>{063_iv_image_viewer_2})。ここでは表紙・裏表紙の元画像を表示してみています。

//image[063_iv_image_viewer_1][起動直後(最初の画像表示)]{
//}

//image[063_iv_image_viewer_2]['j'キー押下で次の画像表示]{
//}
