...
この章では@<img>{5_fs}の構成でファイルシステム関連の機能を実装します。

//image[5_fs][この章で作る機能の構成]{
//}

poibootが配置したファイルシステムを解釈するファイルシステム機能(fs.cとfs.h)と、与えられたファイルからファイルシステムイメージを生成するスクリプト(create_fs.sh)を作成します。

== poibootの第2のロード機能
poibootはkernel.binだけでなく、もうひとつイメージをロードしてくれる機能があります。"fs.img"というファイル名でkernel.binと同じ階層に配置しておくと(@<list>{apps_img_same_floor})、poiboot.confの2行目に記載したアドレスへに配置します。
...
poibootはカーネルへジャンプする際、第3引数にファイルシステムの先頭アドレスを渡してくれるので、カーネルはエントリ関数(main.cのstart_kernel関数)の第3引数からファイルシステムの先頭アドレスを知ることができます。

この章では、ファイルシステム領域上のバイナリ列をファイルと見立てるルールを決めることで、簡易的なファイルシステムとします。

== 単一のテキストファイルを読んでみる
まずはpoibootの機能確認として、単一のテキストファイルをfs.imgというファイル名で配置し、start_kernel関数の第3引数で受け取ったアドレスへファイルの内容がロードされていることを確認してみます。

この節のサンプルディレクトリは"050_fs_read_text"です。これまで同様に文中では前回のサンプルを元に説明しますので、説明に沿って作業を行う場合は前回のサンプル"040_intr"を元に作業を行ってください。
...
次に、start_kernel関数第3引数に指定されたアドレス(fs.imgのロード先アドレス)を読むようにkernel.binのプログラムを改造します。main.cへ@<list>{050_main_c}のように追記します。
...
fs.imgがstart_kernel関数第3引数"_fs_start"へロードされることで、文字列"HELLO\0"が_fs_startに格納されたアドレスから並ぶ事になります。そのアドレスをchar *として参照し、puts関数で出力するだけです。
...
なお、KBC割り込み時のエコーバックは残っているのでキー入力すると"HELLO"に続いて表示されます。

また、"font.c"にアルファベットは大文字しか用意していないのでfs.imgとして小文字のテキストを用意すると化けます。

== ファイルシステムを考える
前節ではfs.imgというファイル名でkernel.binと同じ階層へ配置しておけば、poibootが_fs_startのアドレスへロードされることを確認しました。
...
== ファイルシステムを実装する
それでは、前節で説明したファイルシステムを解釈できるように処理を追加していきます。
...
スクリプトは以下のように、ファイルシステムへ格納したいファイルを引数に指定して実行します。
