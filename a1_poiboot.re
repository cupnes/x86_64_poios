...
問題となったのが、Read関数が返す「実際に読み出したバイト数」です。Lenovo製ThinkPad E450にて、640x480のBGRA画像2枚を格納したapps.img(2.4MB程)をReadさせたとき、「実際に読み出したバイト数」としては指定したapps.imgのサイズを返すのですが、読み出し先を見てみると途中からでたらめなデータが格納されていることがありました。
...
筆者の環境の場合、16KBまでは1回のReadで正しく読み出せていたので「16KBずつ」としていますが、その他のUEFIファームウェアではより小さいサイズしか正しく読み出せない(なおかつ、読み出したサイズは指定された通りのサイズを返す)場合もあるかもしれません。16KBは"063_iv_image_viewer"のkernel.binのサイズでもあるので、その場合はkernel.bin自体正しく読み出せていない可能性があります。そのため、挙動がおかしいがkernel.binやapps.img自体には問題が無いと思われる場合は、poibootでkernel.binとapps.imgを正しくメモリへロードできているか確認してみてください。
...
"kernel body"と"apps"のそれぞれでロードした最初の16バイト("first 16 bytes")と最後の16バイト("last 16 bytes")を出力しています。それらが実際のkernel.binとapps.imgと合っているか確認してください。なお、"kernel body"はkernel.binのヘッダ(BSSの先頭アドレスとサイズ)を除いた部分です。kernel.binの先頭32バイト(ヘッダ)を除いた以降(33バイト目以降)が"kernel body first 16 bytes"に並びますので注意してください。
