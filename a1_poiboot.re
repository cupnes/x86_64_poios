= poibootについて
本書で使用している"poiboot"は前著までの「フルスクラッチで作る!UEFIベアメタルプログラミング」シリーズの知見+αで作成した簡単なブートローダです。
...
前著までで紹介したUEFIファームウェアを直接扱うためのコードは"libuefi"という名前でライブラリ化し、ライブラリ本体のソースコードを"libuefi"ディレクトリに、ヘッダを"include"ディレクトリに配置しています。そして、libuefiを使用してブートローダを作っているpoibootの本体がpoiboot.cです。
...
「4. カーネルへ渡す情報の準備」について、ブートローダからカーネルへ渡す情報はSystemTableとfbの先頭アドレスです。SystemTableはefi_main関数の第2引数で得られるものです。fbはカーネルへフレームバッファの情報を渡すために用意したstruct fb構造体の変数で、init_fb関数で作成しています。
...
ブートローダはUEFIファームウェアから物理アドレスのメモリマップを取得できるので、カーネルが動作し始める時のスタックポインタ設定もブートローダの役割としています。

#@# カーネルは実際に自分自身が動くためにブートローダから受け取ったメモリマップを参考にスタックポインタを再設定しても良いですが、初期状態のメモリマップで今の所問題ないですし、それゆえにまだブートローダはメモリマップをカーネルに渡す実装にもなっていないです。
